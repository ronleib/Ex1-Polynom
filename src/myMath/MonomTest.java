...
public class MonomTest {
	...
	private static void stringConstracorTest() {
		...
		String[] goodMonoms = {"2", "-x","-3.2x^2","0","2x^23333333", "-1","-3.2X^2",""};
		String[] badMoonoms = {"2x^2.5", "--x","-","4x^3.0","(2x^3)", "$4x^2","Boaz","x^22222222222222222222222222222222222222222222222"};
		Monom m = new Monom();
		int counter =0;
		...
		for(int i=0;i<badMoonoms.length;i++) {
			try {
				m = new Monom(badMoonoms[i]);
				String s = m.toString();
				m = new Monom(s);
				double fx = m.f(i);
				System.out.println(i+") "+m +"    \tisZero: "+m.isZero()+"\t f("+i+") = "+fx);}
				...
				System.out.println(counter+1+") "+"Error acurded on "+badMoonoms[counter]);
				counter++;
